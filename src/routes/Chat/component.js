...
import DefaultMessageQueries from "../../components/Messages/DefaultMessageQueries";
			...
			>
				<Screen.Content nopadding>
					{incomingCallAlert && !!incomingCallAlert.show && (
						<CallNotification {...incomingCallAlert} dispatch={dispatch} />
					)}
					{incomingCallAlert?.show &&
					ongoingCall &&
					ongoingCall.callStatus === CallStatus.IN_PROGRESS_SAME_TAB ? (
						<CallIframe {...incomingCallAlert} />
					) : null}
					<div
						className={createClassName(styles, "chat__messages", {
							atBottom,
							loading,
						})}
					>
				...
				{!registrationRequired && !messages.length && (
					<DefaultMessageQueries onQuerySelect={this.handleSubmit} />
				)}
